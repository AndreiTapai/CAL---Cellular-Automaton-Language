...
public class Random {
	...
	public static int random(int[][] range) {
		...
		for (int[] r : range) {
			if (r.length == 1) {
				...
				for (int i = Math.min(r[0], r[1]); i <= Math.max(r[0], r[1]); i++) {
					all[index] = r[0] + i;
					index++;
				}
			}
	 ...
	 * @param length
	 * @param strflag
	 *            It's clumsy, but this is used to differentiate this method
	 *            from the int type random().
	...
	public static char randChar() {
		return (char) (int) (Math.random()
				* ((long) (Character.MAX_VALUE) - (long) (Character.MIN_VALUE + 1)) + (long) (Character.MIN_VALUE));
	}
